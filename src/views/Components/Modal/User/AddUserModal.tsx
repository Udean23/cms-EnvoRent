...
const AddUserModal = ({ isOpen, onClose, onSuccess }: AddUserModalProps) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setError(err?.response?.data?.message || err?.response?.data?.error || "Terjadi kesalahan saat menyimpan karyawan.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <ModalWrapper isOpen={isOpen} onClose={() => { resetForm(); onClose(); }}>
